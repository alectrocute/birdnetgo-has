...
This is a read-only **stats device**: it polls BirdNET-Go's analytics API and never authenticates against your instance, so it cannot change settings, detections, or configuration there.
...
Large `species_list` attributes can generate substantial recorder history. If needed, exclude the summary sensors from recorder; the compact sensors above can remain in history. See [custom dashboard examples](docs/advanced-dashboards.md) and [video scripts](scripts/).
